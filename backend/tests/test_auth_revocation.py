...
def test_password_change_revokes_all_sessions(client, client_factory, sent_emails, db_session):
...
def test_account_deletion_revokes_and_cascades_at_db_level(client, client_factory, sent_emails, db_session):
    user = register_and_verify(client, sent_emails, email="alice@example.com")
    device_b, _ = _second_device(client_factory, "alice@example.com")
...
def test_delete_account_wrong_password_rejected(client, sent_emails):
    user = register_and_verify(client, sent_emails, email="alice@example.com")
    resp = client.request(
        "DELETE",
        "/users/me",
...
def test_password_change_wrong_current_password_rejected(client, sent_emails):
    user = register_and_verify(client, sent_emails, email="alice@example.com")
    resp = client.patch(
        "/users/me/password",
...
def test_email_change_does_not_revoke_sessions(client, client_factory, sent_emails, db_session):
