...
def test_valid_registration_returns_message_only_shape(client, outbox):
...
def test_registration_triggers_verification_email_with_usable_token(client, outbox):
...
def test_new_user_is_unverified_but_can_still_log_in(client, outbox):
...
def test_duplicate_email_does_not_disclose_and_notifies_owner(client, outbox):
    ...
    assert outbox.rows()[0]["to_email"] == "dupe@example.com"


def test_duplicate_email_response_matches_fresh_email_response(client, outbox):
...
def test_duplicate_username_still_rejected_with_400(client, outbox):
