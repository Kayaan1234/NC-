...
def test_login_success_body_shape_has_no_refresh_token(client, outbox):
...
def test_login_sets_refresh_cookie_with_correct_flags(client, outbox):
    _register_alice(client)
    resp = login(client)
...
def test_wrong_password_generic_400(client, outbox):
    _register_alice(client)
...
def test_nonexistent_email_matches_wrong_password_exactly(client, outbox):
