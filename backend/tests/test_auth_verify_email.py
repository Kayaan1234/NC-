...
def test_valid_token_verifies_account_once(client, outbox, db_session):
    register(client, email="alice@example.com")
    token = outbox.token(EmailType.VERIFY_EMAIL)
...
def test_reusing_consumed_token_fails(client, outbox):
    register(client, email="alice@example.com")
    token = outbox.token(EmailType.VERIFY_EMAIL)
...
def test_expired_token_fails(client, outbox, db_session):
    register(client, email="alice@example.com")
    token = outbox.token(EmailType.VERIFY_EMAIL)
...
def test_reset_token_cannot_be_used_on_verify_endpoint(client, outbox, db_session):
